...
def clear_active_parkings(context):
    ...
    for item in vehicles:
        ...
        reg = item['vehicle']['registration_no']
...
def driver_pays(context, amount):
    ...
    response = requests.post(URL + f"/payment/{context.country}/{context.reg_no}", json=json_body)
    assert response.status_code == 200
...
def check_vehicle_removed(context):
    response = requests.delete(URL + f"/entry/{context.country}/{context.reg_no}")

    assert response.status_code == 200
    assert response.json()["status"] is True

    check_resp = requests.get(URL + "/vehicles")
    vehicles = check_resp.json()

    assert not any(v['vehicle']['registration_no'] == context.reg_no for v in vehicles)
...
def check_history(context):
    ...
    found = any(h['vehicle']['registration_no'] == context.reg_no for h in history)
